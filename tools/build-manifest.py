...
for path in sorted(p for p in (root / "Plugins").glob("*.clinkplugin") if not p.name.startswith("._")):
    ...
    plugins.append({"id": path.stem, "name": plugin["name"], "version": plugin.get("version", "latest"), "asset": {"path": path.name, "url": f"https://github.com/{repo}/releases/download/latest/{path.name}", "sha256": hashlib.sha256(data).hexdigest(), "byteCount": len(data)}})
